 ...
 * 脚本地址 https://raw.githubusercontent.com/sofm13/qinlongjs/master/zsq_JS/kgjd.js
 * 
 ...
 * 薅了可能有,不薅一定没有,别问收益!!
 *
 *一键收矿石:这个比较特殊,需要邀请超过10人才能开启,然后有两个地方,一个是跟随任务每天 8 点一次,另一个跟随 红包雨 每小时一次
 * 
 * ========= 青龙--配置文件 =========
 ...
 */
 ...
 //////////////////////
 let ckStr = process.env.kgjd_data;
 let kgjd_dataArr = [];
 let msg = "";
 let ck = "";
 let sign = "";
 ...
 async function start() {
   ...
   h = myDate.getHours();
   // console.log(h);
   console.log(`现在的时间 ${h} 点 ,签到,观看视频,惊喜红包,升级 等任务每天 8 点开启! `);
   if (h == 8) {
 ...
 async function gold_ad_video(timeout = 3 * 1000) {
   ...
   let result = await httpGet(url, `惊喜红包`, timeout);
   if (result.result == "success") {
     console.log(
